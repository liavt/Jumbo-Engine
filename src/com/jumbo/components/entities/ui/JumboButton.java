...
public class JumboButton extends JumboGraphicsObject {
	...
	public TriggeredAction getHoveraction() {
		return this.hovericon.getCustomaction();
	}

	public void setHoveraction(TriggeredAction hoveraction) {
		this.hovericon.setCustomaction(hoveraction);
	}
	...
	public void tick() {
		...
		Rectangle outpos = getOutbounds();
		Rectangle iconbounds = new Rectangle(0, 0, outpos.width, outpos.height);
		Position renderpos;
		if (this.icon != null) {
			...
			this.icon.calculatePosition();
			this.icon.setOutbounds(iconbounds);
			this.icon.tick();
			renderpos = this.icon.getRenderposition();
		} else {
			Rectangle bounds = getInheritedOutbounds();
			renderpos = new Position(bounds.x, bounds.y);
		}
		int x = k.mousex;
		int y = k.mousey;
		Rectangle outbounds = new Rectangle(renderpos.x, renderpos.y, outpos.width, outpos.height);
		boolean clicked = k.leftreleased;
		boolean hovering = x >= outbounds.x && x <= outbounds.width + outbounds.x && y >= outbounds.y
				&& y <= outbounds.height + outbounds.y,
				triggertriggered = this.trigger != null && this.trigger.triggered();
		boolean triggeraction = (clicked && hovering) || triggertriggered, triggerhover = hovering
				|| (this.triggered && triggertriggered) || (this.hoverevent != null && this.hoverevent.triggered());
		boolean buttonactive = !this.outofbounds && this.active && k.inputenabled;
		if (buttonactive) {
			if (triggeraction) {
				if (!this.triggered) {
				...
				if (this.hovericon != null) {
					...
					this.hovericon.calculatePosition();
			...
			if (this.disabledicon != null) {
				...
				this.disabledicon.calculatePosition();
		...
		if (this.descriptor != null) {
			...
			this.descriptor.calculatePosition();
